...
void CLRproj1::Cricket_country::speak()
{
	while (1) {
		...
		int index = hmm.start("Record_Module\\Voice\\test.txt", "Cricket\\Modelpara\\finalA.txt",
			"Cricket\\Modelpara\\finalB.txt", "Cricket\\Modelpara\\finalPI.txt",
			"Cricket\\Modelpara\\codebook.txt", levelsize);
		//int index = 0;
		if (index == -1) {
